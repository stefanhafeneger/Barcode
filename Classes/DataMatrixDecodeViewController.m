//
//  DataMatrixDecodeViewController.m
//  Barcode
//
...
#import "DataMatrixDecodeViewController.h"
...
@interface DataMatrixDecodeViewController ()
#pragma mark Properties
@property(nonatomic, retain) DataMatrixEngine *dataMatrixEngine;
...
@implementation DataMatrixDecodeViewController
...
- (id)initWithNibName:(NSString *)nibName bundle:(NSBundle *)nibBundle {
	...
	if(self != nil) {
		...
		DataMatrixEngine *dataMatrixEngine = [[DataMatrixEngine alloc] init];
		dataMatrixEngine.delegate = self;
		self.dataMatrixEngine = dataMatrixEngine;
		[dataMatrixEngine release];
		self.image = nil;
		self.result = nil;
		self.imageView = nil;
		self.activity = NO;
...
@synthesize dataMatrixEngine = _dataMatrixEngine;
...
- (void)updateInterface {
	if(self.isActivity) {
		...
		buttonDecode.enabled = (self.image != nil && !self.dataMatrixEngine.isRunning);
...
- (IBAction)decodeImage:(id)sender {
	...
	// Decode image.
	[self.dataMatrixEngine decodeImage:self.image withRect:rect];
	
}

- (IBAction)cancelDecode:(id)sender {
	[self.dataMatrixEngine cancelOperation];
}
...
- (void)engine:(DataMatrixEngine *)engine willDecodeImage:(UIImage *)image {
	self.result = nil;
	textView.text = nil;
	self.activity = YES;
	[self updateInterface];
...
- (void)engine:(DataMatrixEngine *)engine didDecodeImage:(UIImage *)image withString:(NSString *)string {
...
- (void)engine:(DataMatrixEngine *)engine didNotDecodeImage:(UIImage *)image {
...
- (void)engineDidCancelOperation:(DataMatrixEngine *)engine {
	self.activity = NO;
	[self updateInterface];
}

- (void)engineDidStopOperation:(DataMatrixEngine *)engine {
	[self updateInterface];
}
